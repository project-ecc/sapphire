...
class OldContactModal extends React.Component {
  constructor(props) {
    super(props);
  ...
  componenDidMount() {
    this.hasBackButton = !!this.props.contactToAdd.multipleAddresses;
  }
  ...
  async handleConfirm() {
    ...
    if (this.state.newContact.address.length > 0 || this.props.foundAnsAddresses.length > 0) {
      const selected = this.state.newContact.address;
...
export default connect(mapStateToProps, actions, null, { withRef: true })(OldContactModal);
